...
const LandingPage: React.FC<LandingPageProps> = ({ 
  onNavigate, onLoginCustomer, onLoginOwner, onLoginAdmin, user, language, setLanguage, theme, setTheme
}) => {
  const t = translations[language];

  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 flex flex-col font-sans selection:bg-orange-100 selection:text-orange-900 overflow-x-hidden transition-colors duration-300">
      
      {/* Mega Navigation */}
      <motion.nav 
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        className="fixed w-full z-50 top-0 bg-white/70 dark:bg-slate-950/70 backdrop-blur-2xl border-b border-slate-100 dark:border-slate-850 py-6 transition-colors duration-300"
      >
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <motion.button 
              ...
              className="px-6 py-3 bg-white dark:bg-slate-950 text-slate-900 dark:text-white border-2 border-slate-900 dark:border-white rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-slate-900 hover:text-white dark:hover:bg-white dark:hover:text-slate-950 transition-all shadow-lg cursor-pointer"
            >
              <Wallet size={16} />
              Connect Wallet
            </motion.button>

            {/* Theme Toggle Button next to wallet */}
            <motion.button 
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')}
              className="p-3 bg-slate-50 dark:bg-slate-900 text-orange-600 dark:text-orange-500 rounded-xl hover:bg-orange-50 dark:hover:bg-slate-800 border-2 border-slate-150 dark:border-slate-800 transition-all shadow-sm flex items-center justify-center cursor-pointer"
            ...
            <motion.button 
              ...
              className="px-4 py-2 hover:bg-slate-50 dark:hover:bg-slate-850 rounded-xl text-[10px] font-black text-slate-900 dark:text-slate-100 border-2 border-slate-100 dark:border-slate-850 flex items-center gap-2 transition-all uppercase tracking-widest cursor-pointer"
            >
              <Languages size={14} className="text-orange-600" />
              {language === 'EN' ? 'हिन्दी' : 'English'}
            </motion.button>
            <div className="h-8 w-px bg-slate-100 dark:bg-slate-850 transition-colors"></div>
            <motion.button 
              whileHover={{ scale: 1.05, rotate: 90 }}
              whileTap={{ scale: 0.95 }}
              onClick={onLoginAdmin}
              className="bg-slate-900 dark:bg-slate-800 text-white p-2.5 rounded-xl hover:bg-slate-800 transition-all shadow-xl border border-transparent dark:border-slate-700 cursor-pointer"
            >
              <Settings size={20} />
            </motion.button>
          </div>
